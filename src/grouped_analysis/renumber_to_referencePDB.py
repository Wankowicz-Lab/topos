...
def to1(resn: str) -> str:
    """3-letter residue name -> 1-letter code (unknown -> X)."""
    try:
...
def build_alignment_params() -> tuple[align.SubstitutionMatrix, tuple[int, float]]:
    """Return BLOSUM62 matrix and affine gap penalty for global alignment."""
...
def get_chain_sequence(df: pd.DataFrame, chain: str) -> list[tuple[int, str]]:
    """Return [(resi, resn_1letter), ...] for the given chain, sorted by resi."""
...
def align_and_map(
    ...
    Returns
    -------
    mapping : dict  query_resi -> ref_resi  (None if query resi is in a gap)
    mismatches : int  number of aligned positions where residue letters differ
    """
...
def renumber_df(df: pd.DataFrame, mapping: dict[int, int | None]) -> pd.DataFrame:
    """Apply resi mapping to resi_struct (and resi_mut if present)."""
    df = df.copy()
...
def detect_pdbs(input_dir: Path, ref_pdb: str = None) -> list[str]:
    """Detect unique PDB IDs present in _features.csv files in the input directory."""
    pdbs = set()
