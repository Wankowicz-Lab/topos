...
class MutationSpec:
    ...
    def __str__(self) -> str:
        # Returns standard mutation notation, e.g. "A123V" (WT aa + residue number + mutant aa)
...
class StructureEntry:
    ...
    def mutation_summary(self) -> str:
        # Returns a comma-separated string of all mutations, e.g. "A123V, G45S", or "" if none
...
def load_config(
    ...
    if raw_pairs:
        ...
        # Auto-pair: no explicit [[pairs]] blocks found, so pair every non-reference
        # structure against the first wt/apo entry (or entries[0] as fallback).
        ref = next(
