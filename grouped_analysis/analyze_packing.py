#!/usr/bin/env python3
"""
Analyze packing scores vs. HBDScan clusters with flexible I/O.

Examples
--------
python packing_vs_clusters.py \
  --csv-glob 'PDBs/PDBs_hold/*_packing.csv' \
  --clusters-csv '/path/to/HBDScan_clusters_3ormore.csv' \
  --clusters-pdb-col PDB \
  --clusters-strip-suffix '_qFit_H.pdb' \
  --output-dir results
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Packing score analysis vs HBDScan clusters")
    # Inputs
    ...
    p.add_argument("--clusters-csv", required=True,
                   help="Path to HBDScan clusters CSV.")
    p.add_argument("--clusters-pdb-col", default="PDB",
                   help="Column in clusters CSV containing PDB filenames/IDs. [default: PDB]")
    p.add_argument("--clusters-strip-suffix", default="_qFit_H.pdb",
                   help="Suffix to strip from clusters PDB column to get base PDB name. "
                        "Use '' to disable. [default: _qFit_H.pdb]")

    # How to extract PDB name from packing CSV filenames
    p.add_argument("--pdb-name-from", choices=["slice", "regex"], default="slice",
                   help="Choose 'slice' (take substring of basename) or 'regex' with a named group 'pdb'.")
    p.add_argument("--pdb-slice", default="0:5",
                   help="For pdb-name-from=slice, python slice spec on basename before extension. [default: 0:5]")
    p.add_argument("--pdb-regex", default=r"^(?P<pdb>[^_]+)",
                   help="For pdb-name-from=regex, a regex with named group 'pdb'. [default: '^(?P<pdb>[^_]+)']")
...
def load_packing_tables(csv_glob: str,
    ...
    for f in files:
        ...
        if packing_col not in df.columns or resi_col not in df.columns:
            raise KeyError(f"Expected columns '{packing_col}' and '{resi_col}' in {f}. Found: {df.columns.tolist()}")
...
def load_clusters(clusters_csv: str,
                  pdb_col: str,
                  strip_suffix: str) -> pd.DataFrame:
    df = pd.read_csv(clusters_csv)
    if pdb_col not in df.columns:
        raise KeyError(f"Column '{pdb_col}' not found in {clusters_csv}. Columns: {df.columns.tolist()}")

    df = df.copy()
    if strip_suffix:
        df["PDB_Name"] = df[pdb_col].astype(str).str.replace(strip_suffix, "", regex=False)
    else:
        # If no suffix stripping, take stem by default
        df["PDB_Name"] = df[pdb_col].astype(str).str.replace(r"\.pdb$", "", regex=True)
    if "Cluster" not in df.columns:
        raise KeyError(f"'Cluster' column not found in {clusters_csv}. Columns: {df.columns.tolist()}")
    return df
...
def main():
    ...
    # Load clusters
    hbd = load_clusters(args.clusters_csv, args.clusters_pdb_col, args.clusters_strip_suffix)

    # Merge packing with cluster label
    merged = pd.merge(avg_by_pdb, hbd[["PDB_Name", "Cluster"]], on="PDB_Name", how="inner")
    ...
    # Boxplot of average packing by HBDScan cluster
    ...
    # Average packing for each (resi_cluster, HBD cluster)
    cluster_stats = []
    ...
    # Heatmap: mean packing by (resi_cluster, HBD cluster)
    if not cluster_stats_df.empty:
        ...
        plt.title("Mean Packing Scores by Residue Cluster and HBDScan Cluster", fontsize=16)
        plt.xlabel("HBDScan Cluster", fontsize=14)
        plt.ylabel("Residue Cluster", fontsize=14)
        savefig(out("residue_cluster_x_hbd_cluster_heatmap.png"), dpi=args.fig_dpi)

        # Boxplots for each (resi_cluster, HBD cluster)
        # Build a tidy dataframe for seaborn instead of manual plt.boxplot
        long_rows = []
        for _, row in cluster_stats_df.iterrows():
            # Reconstruct values per group for plotting
            residues = list(map(int, str(row["Residues"]).split(",")))
            pdbs = hbd.loc[hbd["HBD_Cluster" if "HBD_Cluster" in hbd.columns else "Cluster"] == row["HBD_Cluster"], "PDB_Name"]
            ...
            vals["HBD_Cluster"] = row["HBD_Cluster"]
        ...
        if long_rows:
            ...
            plt.title("Distribution of Packing Scores by Residue Cluster and HBDScan Cluster", fontsize=16)
            plt.xlabel("Residue Cluster (R) x HBDScan Cluster (H)", fontsize=14)
        ...
        pivot_median = cluster_stats_df.pivot(index="Resi_Cluster", columns="HBD_Cluster", values="Median_Packing")
        g = sns.clustermap(
            pivot_median,
            cmap="magma",
            center=0,
