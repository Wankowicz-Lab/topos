...
def _make_aaindex_data(accessions, categories=None):
    """Create a small AAindex DataFrame matching the production CSV schema."""
    n = len(accessions)
    if categories is None:
        categories = [['hydrophobicity', 'volume'][i % 2] for i in range(n)]
    rows = []
    for j, acc in enumerate(accessions):
        row = {'accession': acc, 'description': f'desc{j}', 'category': categories[j]}
        for i, aa in enumerate(AAINDEX_AA_COLUMNS):
            row[aa] = 0.5 * (j + 1) + i * 0.1
        rows.append(row)
