...
def test_calculate_position_effect_quartiles_with_pos_effect():
    ...
    residue_table = residue_table[residue_table['resi_seq'] != 10]
    new_row = pd.DataFrame({'chain': ['A'], 'resi_seq': [10], 'resn_seq': ['ALA'], 'resi_struct': [10], 'resn_struct': ['ALA'], 'resm': [np.nan],
    ...
    pos_effects = residue_table.groupby('resi_seq')['effect'].mean().reset_index()
    pos_effects.rename(columns={'effect': 'pos_effect'}, inplace=True)
    residue_table = pd.merge(residue_table, pos_effects, on='resi_seq', how='left')
    ...
    assert 10 not in quartile_df.resi_seq.values
...
def test_calculate_aaindex_scores_no_muts():
    ...
    for acc in accessions:
        ...
        for idx, row in aaindex_df.iterrows():
            # When there are no mutations, the column is resn_struct instead of resn_seq
            resn_col = 'resn_seq' if 'resn_seq' in row.index else 'resn_struct'
            expected_value = feature_values.get(row[resn_col], np.nan)
...
def test_calculate_aaindex_scores_with_muts():
    ...
    for acc in accessions:
        feature_values = aaindex_data.set_index('accession').loc[acc].iloc[1:]
        for idx, row in aaindex_df.iterrows():
            expected_wt = feature_values.get(row['resn_seq'], np.nan)
...
def test_calculate_blosum_score():
    ...
    for idx, row in blosum_df.iterrows():
        expected_score = b_matrix[convert_amino_acid(row['resn_seq'])][convert_amino_acid(row['resm'])]
