...
from topos.metrics import secondary_structure


def test_ss_domain_lengths():
    """ss_domain_lengths returns ss_domains and ss_domain_length with correct counts."""
    merged = pd.DataFrame({
    ...
    assert list(out.columns) == ['chain', 'ss_domains', 'ss_domain_length']
    assert out['ss_domain_length'].tolist() == [2, 2, 2, 1]
...
def test_ss_domain_log2_aa_group_ratios():
    ...
    assert d1['ss_domain_log2_aa_group_ratio_Nonpolar_Aliphatic'] == pytest.approx(np.log2(1.0 / (3/4)))
    assert d1['ss_domain_log2_aa_group_ratio_Aromatic'] == -np.inf
    assert d2['ss_domain_log2_aa_group_ratio_Nonpolar_Aliphatic'] == pytest.approx(np.log2(0.5 / (3/4)))
    assert d2['ss_domain_log2_aa_group_ratio_Aromatic'] == pytest.approx(np.log2(0.5 / (1/4)))
    
    # B1 (chain B): 1 Nonpolar_Aliphatic, 0 Aromatic -> prop 1/1, 0/1
    assert b1['ss_domain_log2_aa_group_ratio_Nonpolar_Aliphatic'] == pytest.approx(np.log2(1.0 / (1/1)))
    assert pd.isna(b1['ss_domain_log2_aa_group_ratio_Aromatic'])  # prop_global=0 for Aromatic in chain B
